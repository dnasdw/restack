...
| `10-force-upgrade.sh` | `10-force-upgrade.sh <base> <branch>` | Force ONE already-upgraded (idempotent) branch through a fresh bridge + cherry-pick cycle and update its `last-base` to the new bridge, so the replayed commits are brand-new local history you may rewrite freely. Refuses branches not yet on their base. Auto-backup before and after; the post snapshot always fires |

Every numbered script (`01-init.sh` through `10-force-upgrade.sh`) accepts `-h` / `--help` to print its usage and exit 0. `lib.sh` is a library, not a runnable script.
